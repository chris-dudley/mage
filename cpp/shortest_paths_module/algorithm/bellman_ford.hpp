...
class BellmanFordPathfinder {
 ...
 public:
  ...
  /// @param check_abort Function that should throw an exception if execution should be aborted.
  /// @throws std::invalid_argument if the source node is not in the graph.
  BellmanFordPathfinder(const GraphViewType &graph, TSize source, const EdgeIdSet &ignored_edges,
                        const NodeIdSet &ignored_nodes, const CheckAbortFunc &check_abort = CheckAbortNoop)
      : num_vertex(graph.Nodes().size()),
        source_id(source),
        dist_to(graph.Nodes().size(), POS_INF),
        edge_into(graph.Nodes().size(), std::nullopt),
  ...
  bool has_negative_cycle() const noexcept { return cycle.has_value(); }

  /// @brief Returns a found negative cycle, if any.
  std::optional<Path<TSize>> negative_cycle() const { return cycle; }
  ...
  const PredecessorVec &predecessors() const { return edge_into; }

  /// @brief Returns the vector containing the shortest distance from the source vertex to each vertex
  /// in the graph.
  const std::vector<double> &distances() const { return dist_to; }
 ...
 private:
  ...
  void do_search(const GraphViewType &graph, TSize source, const EdgeIdSet &ignored_edges,
                 const NodeIdSet &ignored_nodes, const CheckAbortFunc &check_abort) {
    dist_to[source] = 0.0;
  ...
  bool find_negative_cycle() {
    ...
    for (size_t vertex = 0; vertex < num_vertex; vertex++) {
      if (edge_into[vertex]) {
        ...
        graph.CreateEdge(edge.from, edge.to, mg_graph::GraphType::kDirectedGraph, std::make_optional(edge.id), true,
                         edge.weight);
      }
    }
...
template <typename TSize = std::uint64_t>
Path<TSize> BellmanFord(const mg_graph::GraphView<TSize> &graph, TSize source_id, TSize target_id,
                        const std::unordered_set<TSize> &ignored_edges, const std::unordered_set<TSize> &ignored_nodes,
