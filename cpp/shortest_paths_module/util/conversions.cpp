...
EdgeNetwork<uint64_t> EdgeNetworkFromMemgraph(const mgp::Graph &graph, std::vector<mgp::Path> paths,
                                              ...
                                              const std::string &y_coord_prop_name, bool fit_2point_quadratic) {
  ...
  for (size_t path_idx = 0; path_idx < paths.size(); path_idx++) {
    ...
    for (size_t edge_idx = 0; edge_idx < path.Length(); edge_idx++) {
      ...
      if (!x_coords_value.IsList()) {
        throw std::invalid_argument(fmt::format("path {}, edge {} (id: {}) property '{}' is not a list", path_idx,
                                                edge_idx, edge_id, x_coord_prop_name));
      }
      ...
      for (auto x_value : x_coords_list) {
        if (x_value.IsNumeric()) {
          ...
          throw std::invalid_argument(
              fmt::format("path {}, edge {} (id: {}) property '{}' contains non-numeric value at index {}", path_idx,
                          edge_idx, edge_id, x_coord_prop_name, x_coords.size()));
        }
      }
      ...
      if (!y_coords_value.IsList()) {
        throw std::invalid_argument(fmt::format("path {}, edge {} (id: {}) property '{}' is not a list", path_idx,
                                                edge_idx, edge_id, y_coord_prop_name));
      }
      for (auto y_value : x_coords_list) {
        if (y_value.IsNumeric()) {
          ...
          throw std::invalid_argument(
              fmt::format("path {}, edge {} (id: {}) property '{}' contains non-numeric value at index {}", path_idx,
                          edge_idx, edge_id, y_coord_prop_name, y_coords.size()));
        }
      }
      ...
      cur_path.emplace_back(edge_id, x_coords, y_coords, fit_2point_quadratic);
    }
