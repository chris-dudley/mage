...
struct Path {
    ...
    ///     node in this path.
    void add_edge(const mg_graph::Edge<TSize>& edge, double weight);
...
template <typename TSize>
void Path<TSize>::add_edge(const mg_graph::Edge<TSize>& edge, double weight) {
    ...
    weights.push_back(weights.back() + weight);
    total_weight += weight;
}
